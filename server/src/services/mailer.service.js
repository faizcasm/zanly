...
const sendMail = async ({ Otp, email, name }) => {
  const info = await transporter.sendMail({
    from: `"Zanly" <${process.env.GMAIL_USER}>`,
    to: email,
    subject: `Hello ${name}, here is your OTP ✔`,
    text: `Your OTP is ${Otp} for Zanly (valid for 10 minutes).`,
    html: generateOtpTemplate(Otp, 'Zanly'),
  });
